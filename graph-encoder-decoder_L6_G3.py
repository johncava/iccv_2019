...
class GraphEncoderDecoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.a2 = nn.Linear(2*12,1)
        self.W2 = nn.Linear(12,12)
    ...
    def forward(self,x,g):
        ...
        h = list(h.unsqueeze(dim=1))
        h2 = []
        for i,hi in enumerate(h):
            a_weights = []
            h_neighbors = []
            for j, hj in enumerate(h):
                a = F.relu(self.a2(torch.cat((self.W2(hi),self.W2(hj)),dim=1)))
                a_weights.append(a)
                h_neighbors.append(hj)
            s = []
            for aw, hn in zip(a_weights,h_neighbors):
                s.append(self.W2(aw*hn))
            h2.append(sum(s))
        h2 = torch.cat(h2,dim=0)
        
        h2 = list(h2.unsqueeze(dim=1))
        h3 = []
        for i,hi in enumerate(h2):
            a_weights = []
            h_neighbors = []
            for j, hj in enumerate(h2):
                a = F.relu(self.a3(torch.cat((self.W3(hi),self.W3(hj)),dim=1)))
                a_weights.append(a)
                h_neighbors.append(hj)
            s = []
            for aw, hn in zip(a_weights,h_neighbors):
                s.append(self.W3(aw*hn))
            h3.append(sum(s))
